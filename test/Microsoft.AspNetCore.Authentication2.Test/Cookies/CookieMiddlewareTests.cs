            ...
            var builder = new WebHostBuilder()
                .Configure(app =>
                    app.Map("/base", map =>
                    {
                        ...
                        map.Map("/login", signoutApp => signoutApp.Run(context => context.ChallengeAsync("Cookies", new AuthenticationProperties2() { RedirectUri = "/" })));
                    }))
                .ConfigureServices(services => services.AddCookieAuthentication(o => o.LoginPath = new PathString("/page")));
            var server = new TestServer(builder);
        ...
        public async Task RedirectUriIsHoneredAfterSignin(string redirectUrl)
        {
            ...
            async context =>
                await context.SignInAsync(
                    ...
                    new AuthenticationProperties2 { RedirectUri = redirectUrl })
            );
            var transaction = await SendAsync(server, "http://example.com/testpath");
        ...
        public async Task EnsurePrecedenceOfRedirectUriAfterSignin()
        {
            ...
            async context =>
            {
                await context.SignInAsync(
                    CookieAuthenticationDefaults.AuthenticationScheme,
                    new ClaimsPrincipal(new ClaimsIdentity(new GenericIdentity("Alice", CookieAuthenticationDefaults.AuthenticationScheme))),
                    new AuthenticationProperties2 { RedirectUri = "/redirect_test" });
            });
            var transaction = await SendAsync(server, "http://example.com/testpath?return=%2Fret_path_2");
            ...
            var builder = new WebHostBuilder()
                .Configure(app =>
                {
                    ...
                    app.Use(async (context, next) =>
                    {
                        ...
                        {
                            await context.ChallengeAsync(CookieAuthenticationDefaults.AuthenticationScheme, new AuthenticationProperties2(), ChallengeBehavior.Unauthorized);
                        }
                        else if (req.Path == new PathString("/protected/CustomRedirect"))
                        {
                            await context.ChallengeAsync(CookieAuthenticationDefaults.AuthenticationScheme, new AuthenticationProperties2() { RedirectUri = "/CustomRedirect" });
                        }
