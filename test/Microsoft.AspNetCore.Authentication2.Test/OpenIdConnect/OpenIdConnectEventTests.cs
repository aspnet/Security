...
namespace Microsoft.AspNetCore.Authentication2.Test.OpenIdConnect
{
    public class OpenIdConnectEventTests
    {
        ...
        public async Task OnAuthenticationFailed_HandledWithTicket_SkipToTicketReceived()
        {
            ...
            var server = CreateServer(new OpenIdConnectEvents()
            {
                ...
                OnAuthenticationFailed = context =>
                {
                    ...
                    context.Ticket = new AuthenticationTicket2(
                        new ClaimsPrincipal(new ClaimsIdentity(claims, context.Options.AuthenticationScheme)),
                        new AuthenticationProperties2(), context.Options.AuthenticationScheme);
        ...
        private class TestStateDataFormat : ISecureDataFormat<AuthenticationProperties2>
        {
            private AuthenticationProperties2 Data { get; set; }

            public string Protect(AuthenticationProperties2 data)
            {
                throw new NotImplementedException();
            }

            public string Protect(AuthenticationProperties2 data, string purpose)
            {
                throw new NotImplementedException();
            }

            public AuthenticationProperties2 Unprotect(string protectedText)
            {
                Assert.Equal("protected_state", protectedText);
                return new AuthenticationProperties2(new Dictionary<string, string>()
                {
                    { ".xsrf", "corrilationId" },
            ...
            public AuthenticationProperties2 Unprotect(string protectedText, string purpose)
            {
                throw new NotImplementedException();
            }
        }
