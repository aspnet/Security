...
namespace Microsoft.AspNetCore.Authentication.Tests.VsoAccount
{
    public class VsoAccountTests
    {
        private void ConfigureDefaults(VsoAccountOptions o)
        {
            o.ClientId = "whatever";
        ...
        public async Task CanForwardDefault()
        {
            ...
            {
                o.DefaultScheme = VsoAccountDefaults.AuthenticationScheme;
        ...
        public async Task VerifySignInSchemeCannotBeSetToSelf()
        {
            var server = CreateServer(o =>
            {
                ...
                o.SignInScheme = VsoAccountDefaults.AuthenticationScheme;
            });
        ...
        public async Task VerifySchemeDefaults()
        {
            ...
            services.AddAuthentication().AddVsoAccount();
            var sp = services.BuildServiceProvider();
            var schemeProvider = sp.GetRequiredService<IAuthenticationSchemeProvider>();
            var scheme = await schemeProvider.GetSchemeAsync(VsoAccountDefaults.AuthenticationScheme);
            Assert.NotNull(scheme);
            Assert.Equal("VsoAccountHandler", scheme.HandlerType.Name);
            Assert.Equal(VsoAccountDefaults.AuthenticationScheme, scheme.DisplayName);
        }
        ...
        public async Task ChallengeWillTriggerRedirection()
        {
            ...
            Assert.Contains("https://app.vssps.visualstudio.com/oauth2/authorize", location);
            Assert.Contains("response_type=Assertion", location);
        ...
        public async Task AuthenticatedEventCanGetRefreshToken()
        {
            ...
            var server = CreateServer(o =>
            {
                ...
                o.BackchannelHttpHandler = new TestHttpMessageHandler
                {
                    Sender = req =>
                    {
                        if (req.RequestUri.AbsoluteUri == "https://app.vssps.visualstudio.com/oauth2/token?mkt=en-US")
                        {
                ...
                o.Events = new OAuthEvents
                {
                    OnCreatingTicket = context =>
                    {
                        ...
                        context.Principal.AddIdentity(new ClaimsIdentity(new Claim[] { new Claim("RefreshToken", refreshToken, ClaimValueTypes.String, "Vso") }, "Vso"));
            ...
            var transaction = await server.SendAsync(
                "https://example.com/signin-vso?code=TestCode&state=" + UrlEncoder.Default.Encode(state),
                $".AspNetCore.Correlation.Vso.{correlationValue}=N");
            Assert.Equal(HttpStatusCode.Redirect, transaction.Response.StatusCode);
            Assert.Equal("/me", transaction.Response.Headers.GetValues("Location").First());
            Assert.Equal(2, transaction.SetCookie.Count);
            Assert.Contains($".AspNetCore.Correlation.Vso.{correlationValue}", transaction.SetCookie[0]);
        ...
        private static TestServer CreateServer(Action<VsoAccountOptions> configureOptions)
        {
            var builder = new WebHostBuilder()
                .Configure(app =>
                {
                    ...
                    app.Use(async (context, next) =>
                    {
                        ...
                        {
                            await context.ChallengeAsync("Vso");
                        }
                        ...
                        {
                            await Assert.ThrowsAsync<InvalidOperationException>(() => context.SignInAsync("Vso", new ClaimsPrincipal()));
                        }
                        else if (req.Path == new PathString("/signOut"))
                        {
                            await Assert.ThrowsAsync<InvalidOperationException>(() => context.SignOutAsync("Vso"));
                        }
                        else if (req.Path == new PathString("/forbid"))
                        {
                            await Assert.ThrowsAsync<InvalidOperationException>(() => context.ForbidAsync("Vso"));
                        }
                        else
                        {
                            await next();
                ...
                .ConfigureServices(services =>
                {
                    services.AddAuthentication(TestExtensions.CookieAuthenticationScheme)
                        ...
                        .AddVsoAccount(configureOptions);
                });
